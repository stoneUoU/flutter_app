...
class BannerView extends StatefulWidget {
	...
	BannerView(this.bannerStories, this.onTap, {Key key})
	  :super(key: key);
...
class _BannerViewState extends State<BannerView> {
	...
	List<Widget> _buildItems() { // 排列轮播数组
		...
		if (widget.bannerStories.length > 0) {
			...
			// 正常添加Item
			items.addAll(
			  widget.bannerStories.map((carouselMs) => _buildItem(carouselMs)).toList(
				growable: false));
			// 尾部
			items.add(
	...
	Widget _buildItem(CarouselChildMs carouselMs) {
		return GestureDetector(
			onTap: () { // 按下
				if (widget.onTap != null) {
					widget.onTap(carouselMs);
				}
			},
			child: Stack(
				fit: StackFit.expand,
				children: <Widget>[
					Image.network(carouselMs.imagePath, fit: BoxFit.cover),
					_buildItemTitle(carouselMs.title), // 内容文字,大意
				],),);
	}
...
typedef void OnTapBannerItem(CarouselChildMs carouselMs);